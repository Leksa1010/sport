...
fetch('http://localhost:8000/api/coach')
    ...
    .then(data => {
        data.forEach(coach => {
            ...
            copy.querySelector('.created').innerText = new Date(coach.createdAt).toLocaleString('sr-RS')
            copy.querySelector('.created').innerText = new Date(coach.updatedAt).toLocaleString('sr-RS')
